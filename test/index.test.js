...
describe('Feathers Couchbase Service', () => {
  before(() =>
    //Init CB Connection or lets simply even better use the Mock Server
  );

  describe('Initialization', () => {
    it('throws an error when missing options', () => {
      expect(service.bind(null)).to.throw('Couchbase options have to be provided');
    });

    it('throws an error when missing a Model', () => {
      expect(service.bind(null, { name: 'Test' })).to.throw(/You must provide a Couchbase Model/);
    });
  });
  ...
  describe('Feathers-Couchbase Specific Tests', () => {
    const app = feathers()
      .use('/people', service({
        Model,
        paginate: {
    ...
    describe('Association Tests', () => {
      ...
      it('find() returns correct total when using includes for non-raw requests (#137)', () => {
        const options = {Couchbase: {raw: false, include: Order}};
      ...
      it('find() returns correct total when using includes for raw requests', () => {
        const options = {Couchbase: {include: Order}};
        return people.find(options).then(result => {
          assert.equal(result.total, 2);
        });
      });
    ...
    describe('Custom getters and setters', () => {
      ...
      it('can ignore custom getters and setters (#113)', () => {
        ...
        const service = app.service('custom-getter-setter');
        const data = {addsOneOnGet: value, addsOneOnSet: value};
        const IGNORE_SETTERS = {Couchbase: {ignoreSetters: true}};
    ...
    it('can set the scope of an operation#130', () => {
      ...
      const SCOPE_TO_ACTIVE = {Couchbase: {scope: 'active'}};
      const SCOPE_TO_PENDING = {Couchbase: {scope: 'pending'}};
  ...
  describe('ORM functionality', () => {
    ...
    describe('Non-raw Service Method Calls', () => {
      const _ids = {};
      const _data = {};
      const NOT_RAW = {Couchbase: {raw: false}};
