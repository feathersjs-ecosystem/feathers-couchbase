...
class CouchbaseService extends AdapterService {
  constructor (options = {}) {
    ...
    this.scopeName = options.scope || '_default';
    this.collectionName = options.collection || '_default';
    this.collection = `\`${this.options.name}\`.${this.scopeName}.\`${this.collectionName}\` as c`;
    this.Model = this.bucket.scope(this.scopeName).collection(this.collectionName);
  }
  ...
  getWhere (params) {
    ...
    const objectToQuery = obj => Object.keys(obj).map(field => {
      ...
      if (typeof value === 'object') {
        return Object.keys(value).map(qualifier => {
          ...
          return `c.${field} ${mapping[qualifier]} $${placeholder}`;
        }).join(' AND ');
      }

      const placeholder = parameters.push(value);

      return `c.${field} == $${placeholder}`;
    }).join(' AND ');
    ...
    if (filters.$sort) {
      where += 'ORDER BY ' + Object.keys(filters.$sort).map(field => {
        ...
        return `c.${field} ${value === 1 ? 'ASC' : 'DESC'}`;
      }).join(' , ');
    }
  ...
  getSelect (query = {}) {
    ...
    return fields.map(name => `c.${name}`).join(', ');
  }
  ...
  async _find (params = {}) {
    ...
    const query = `SELECT ${this.getSelect(params.query)} FROM ${this.collection} ${where};`;
    const { rows: data } = await this.query(query, {
      parameters
    });

    if (paginate.default !== undefined) {
      const countQuery = `SELECT COUNT(${this.id}) AS count FROM ${this.collection} ${count}`;
  ...
  async _get (id, params = {}) {
    ...
    const query = `SELECT ${this.getSelect(params.query)} FROM ${this.collection} USE KEYS '${id}' ${where}`;
  ...
  async _create (data, params = {}) {
    ...
    const id = data[this.id] || uuidv4();
    const doc = {
      ...data,
      [this.id]: id
    };
    const q = `INSERT INTO ${this.collection} (KEY, VALUE) VALUES ("${id}", $1) RETURNING ${this.getSelect(params.query)}`;
